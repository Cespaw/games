...
function MultiGame() {
  ...
  function spawnObstacles() {
    if (Math.random() < OBSTACLE_FREQUENCY &&
      ...
      else if (obstacles[obstacles.length - 1].x < GAME_WIDTH / 2) {
        if (Math.random() <= 0.2) {
          const x = GAME_WIDTH
  ...
  function drawLogic() {
    ...
    if (!gameIsStarted) {
      ...
      ctx.fillStyle = 'white'
      const hpLeft = HEALTH_POINTS - hits.left
      if (hpLeft >= 10) {
        ctx.fillText(hpLeft, leftPlayerPos.x + 5, leftPlayerPos.y + 35)
      } else if (hpLeft > 0){
        ctx.fillText(hpLeft, leftPlayerPos.x + 15, leftPlayerPos.y + 35)
      }
      ...
      ctx.fillStyle = 'white'
      const hpMiddle = HEALTH_POINTS - hits.middle
      if (hpMiddle >= 10) {
        ctx.fillText(hpMiddle, middlePlayerPos.x + 5, middlePlayerPos.y + 35)
      } else if (hpMiddle > 0){
        ctx.fillText(hpMiddle, middlePlayerPos.x + 15, middlePlayerPos.y + 35)
      }
      ...
      ctx.fillStyle = 'white'
      const hpRight = HEALTH_POINTS - hits.right
      if (hpRight >= 10) {
        ctx.fillText(hpRight, rightPlayerPos.x + 5, rightPlayerPos.y + 35)
      } else if (hpRight > 0) {
        ctx.fillText(hpRight, rightPlayerPos.x + 15, rightPlayerPos.y + 35)
      }
